    ...
    $.get('/api/stargaze/' + cityChoice, function (results) { // Call to API route using stargazing-time / OpenWeather for city
      if (results.length == 0) {
        ...
        for (i = 0; i < results.length; i++) { // Shows cards if there is result data for them
          ...
          const weatherArray = Object.values(results[i].forecast.weather[0]);
          ...
          const utcSucks = moment.tz(results[i].from, 'Europe/London'); // Converts UTC data pulled back in CDT timezone
          const tzConvert = utcSucks.clone().tz('America/Chicago');
          console.log(tzConvert);

          const day = moment(tzConvert).format('dddd'); // Pulls day of the week back from time
          const timeOfDay = moment(tzConvert).format('MMMM Do, h:mm:ss a');
          console.log(day);
